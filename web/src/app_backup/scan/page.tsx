...
function ScanContent() {
  ...
  const handleSave = () => {
    if (targetAnimalId && isSavingToFarm && result) {
      const stored = localStorage.getItem('mockAnimals');
      if (stored) {
        let allData = JSON.parse(stored);
        if (allData[targetAnimalId]) {
          const newHistoryEntry = {
            attempt: allData[targetAnimalId].history.length + 1,
            date: "วันนี้",
          ...
          allData[targetAnimalId].history.unshift(newHistoryEntry);
          if (base64Image) {
            allData[targetAnimalId].image = base64Image;
          }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f0fdf4] to-[#dcfce7] text-gray-800 flex flex-col relative overflow-hidden">
      
      {/* Decorative background elements */}
      <div className="absolute top-[-20%] right-[-10%] w-96 h-96 bg-emerald-200 rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
      ...
      {showGuidelines && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[32px] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 relative">
            ...
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-[#144A29] to-emerald-700 p-5 pt-8 text-center relative">
              <button 
              ...
              <p className="text-emerald-100 text-sm">ขั้นตอนการถ่ายภาพให้ AI วิเคราะห์ได้แม่นยำ</p>
            </div>
            ...
            <div className="p-6 space-y-5">
              <ul className="space-y-4">
                ...
                <li className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-sm shrink-0">4</div>
              ...
              <button 
                onClick={() => setShowGuidelines(false)}
                className="w-full py-3.5 bg-[#144A29] text-white font-bold rounded-xl hover:bg-emerald-800 transition-colors shadow-md mt-2"
              >
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[32px] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 p-8 flex flex-col items-center text-center relative">
            ...
            <h2 className="text-2xl font-black text-[#064e3b] mb-2">บันทึกสำเร็จ!</h2>
            ...
            <div className="w-full space-y-3">
              <button 
                ...
                className="w-full py-4 bg-[#064e3b] text-white font-bold rounded-2xl hover:bg-[#047857] transition-all shadow-md"
              >
              ...
              <button 
                ...
                className="w-full py-4 bg-gray-100 text-[#064e3b] font-bold rounded-2xl hover:bg-gray-200 transition-all"
              >
                ดูประวัติการชั่งรวม
              </button>
            </div>
      ...
      {/* Header */}
      <header className="bg-white/40 backdrop-blur-md text-[#064e3b] p-4 flex items-center justify-between z-10 border-b border-white/50">
        <div className="flex items-center space-x-4">
          <Link href="/dashboard" className="p-2.5 bg-white/60 shadow-sm rounded-full hover:bg-white transition-all active:scale-95 text-[#047857]">
            <ArrowLeft size={22} />
          </Link>
      ...
      <div className="flex-1 p-5 flex flex-col items-center z-10 overflow-y-auto pb-10">
        ...
        <div className="w-full max-w-sm glass-panel rounded-3xl overflow-hidden flex flex-col transition-all duration-300">
          <div className="p-4 bg-white/40 border-b border-white/50 flex justify-between items-center">
            <h2 className="font-bold text-[#064e3b] flex items-center gap-2">
              <Camera size={18} />
          ...
          {!selectedImage && (
            <div className="px-5 pt-5 pb-1 bg-gradient-to-b from-white/40 to-white/30">
              <label className="block text-sm font-bold text-[#064e3b] mb-2">เลือกประเภทสัตว์</label>
              <div className="flex bg-white rounded-xl p-1 border border-emerald-100 shadow-sm">
                <button 
                  onClick={() => setSelectedType('โคเนื้อ')}
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${selectedType === 'โคเนื้อ' ? 'bg-[#064e3b] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                  โคเนื้อ
                </button>
                <button 
                  onClick={() => setSelectedType('กระบือ')}
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${selectedType === 'กระบือ' ? 'bg-[#064e3b] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                  กระบือ
                </button>
              </div>
          ...
          <div className="flex-1 min-h-[320px] flex flex-col items-center justify-center p-5 relative bg-gradient-to-b from-white/30 to-transparent">
            {selectedImage ? (
              <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-inner border border-white/50">
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-[#064e3b]/80 backdrop-blur-sm flex flex-col items-center justify-center text-white transition-all duration-300">
                    ...
                    <p className="text-sm font-light text-emerald-200 mt-2">จำแนกประเภทและสัดส่วน</p>
              ...
              <div className="w-full flex gap-3 h-full">
                {/* Upload from Gallery */}
                <label className="flex-1 flex flex-col items-center justify-center h-full border-2 border-dashed border-[#047857]/40 rounded-2xl cursor-pointer hover:bg-white/50 hover:border-[#047857] transition-all bg-white/30 group">
                  <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-3 text-[#047857] group-hover:scale-110 transition-transform shadow-sm">
                    <UploadCloud size={32} />
                  </div>
                  <span className="text-[#064e3b] font-bold">อัปโหลดรูปภาพ</span>
                ...
                <button 
                  onClick={startCamera}
                  className="flex-1 flex flex-col items-center justify-center h-full border-2 border-dashed border-[#047857]/40 rounded-2xl cursor-pointer hover:bg-white/50 hover:border-[#047857] transition-all bg-white/30 group"
                >
                  <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-3 text-[#047857] group-hover:scale-110 transition-transform shadow-sm">
                    <Camera size={32} />
                  </div>
                  <span className="text-[#064e3b] font-bold">ถ่ายภาพจากกล้อง</span>
                </button>
              </div>
            )}
          </div>
            ...
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 text-lg shadow-lg ${
                !selectedImage || isProcessing 
                  ...
                  : 'bg-gradient-to-r from-[#064e3b] to-[#047857] text-white hover:opacity-95 active:scale-[0.98]'
              }`}
            >
              {isProcessing ? (
                <>
        ...
        {/* Result Area */}
        {result && (
          <div className="w-full max-w-sm mt-6 glass-panel rounded-3xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 shadow-xl border border-emerald-300 relative">
             <div className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white p-4 pb-8 text-center font-bold flex flex-col items-center justify-center shadow-inner relative">
             ...
             <div className="absolute top-11 left-1/2 -translate-x-1/2 bg-white px-6 py-2 rounded-full shadow-md border-2 border-emerald-400 flex items-center gap-2 text-[#064e3b] font-bold z-10 w-max">
             ...
             <div className="p-6 pt-10 space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white p-3 rounded-2xl border border-gray-100 text-center shadow-sm relative overflow-hidden flex flex-col justify-center">
                    ...
                    <p className="text-xl font-bold text-[#064e3b]">{result.weight}</p>
                    <p className="text-[10px] text-gray-400">KG</p>
                  </div>
                  <div className="bg-emerald-50 p-3 rounded-2xl border border-emerald-100 text-center shadow-sm relative overflow-hidden flex flex-col justify-center">
                    <p className="text-[10px] text-emerald-700 font-bold mb-1">รอบอก (AI)</p>
                    <p className="text-xl font-bold text-emerald-700">{result.aiGirth}</p>
                    <p className="text-[10px] text-emerald-600/70">CM</p>
                  </div>
                  <div className="bg-white p-3 rounded-2xl border border-gray-100 text-center shadow-sm relative overflow-hidden flex flex-col justify-center">
                    <p className="text-[10px] text-gray-500 font-bold mb-1">ส่วนสูง (AI)</p>
                    <p className="text-xl font-bold text-[#064e3b]">{result.height}</p>
                ...
                <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 space-y-4">
                  <div>
                    <label className="block text-sm font-bold text-[#064e3b] mb-2 flex items-center gap-2">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#047857] focus:border-transparent outline-none transition-all font-bold text-[#064e3b]"
                    />
                    ...
                    {realGirth && (
                      <div className="mt-4 pt-4 border-t border-emerald-200 space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-bold text-[#064e3b]">{calculatedWeight} kg</span>
                        </div>
                        <div className="flex justify-between items-center">
                  ...
                  <div className="pt-4 border-t border-emerald-200">
                    ...
                    <div className="flex bg-white rounded-xl p-1 border border-gray-200 mb-4 shadow-sm">
                      <button 
                        ...
                        className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${isSavingToFarm ? 'bg-[#064e3b] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
                      >
                        เก็บเข้าฟาร์ม
                      </button>
                      <button 
                    ...
                    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                      {targetAnimalId ? (
                        <div className="w-full px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between text-sm">
                          <span className="font-bold text-[#064e3b]">อัปเดตข้อมูลสัตว์: #{targetAnimalId}</span>
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#047857] focus:border-transparent outline-none transition-all text-sm"
                        />
                      )}
                    </div>
                  </div>
                ...
                {/* Save Button */}
                {isSavingToFarm ? (
                  <button onClick={handleSave} className="w-full py-4 bg-[#064e3b] text-white font-bold text-lg rounded-2xl hover:bg-[#047857] transition-all shadow-md active:scale-[0.98] flex items-center justify-center gap-2 mt-4">
                    <Save size={20} />
        ...
        <div className="fixed inset-0 z-[100] bg-black flex flex-col">
          ...
          <div className="relative flex-1 flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center bg-black/80 portrait:flex landscape:hidden z-10 backdrop-blur-sm">
              <div className="text-white text-center p-6 flex flex-col items-center">
                <Smartphone className="w-20 h-20 animate-[spin_3s_ease-in-out_infinite] mb-6 text-emerald-400" />
                <p className="text-2xl font-bold mb-2">กรุณาหมุนโทรศัพท์เป็นแนวนอน</p>
                <p className="text-emerald-100/80">เพื่อให้ AI วิเคราะห์สัดส่วนได้อย่างแม่นยำ</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/90 h-28 flex items-center justify-between px-10 pb-safe">
            ...
            <button 
              ...
              className="w-20 h-20 rounded-full bg-white border-4 border-emerald-500 shadow-[0_0_20px_#10b981] active:scale-90 transition-transform focus:outline-none"
            >
...
export default function ScanPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f0fdf4] flex items-center justify-center"><Loader2 className="animate-spin text-[#064e3b]" size={40} /></div>}>
      <ScanContent />
    </Suspense>
  );
}
